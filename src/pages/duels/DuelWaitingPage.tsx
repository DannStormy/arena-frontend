  ...
  useDuelSocket(activeDuel?.id ?? null, {
    onDuelReady: (payload: DuelReadyPayload) => {
      if (activeDuel) goToGame(activeDuel);
        ...
        // duelReady came before duel state was set — navigate using payload id
  ...
  return (
    <div className="flex flex-col min-h-svh bg-arena-bg px-4 pt-safe-top">
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-8">
        {/* Duel code */}
        <div className="rounded-xl border border-arena-border bg-arena-surface p-6 w-full text-center space-y-3">
          <p className="text-white/50 text-xs uppercase tracking-wider">Duel code</p>
          ...
          <div className="flex gap-2 justify-center pt-1">
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-arena-border px-3 py-1.5 text-xs text-white/60 hover:text-white hover:bg-white/5 transition-colors"
            >
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="relative">
            <div className="h-16 w-16 rounded-full border-2 border-arena-border flex items-center justify-center">
              <span className="text-2xl">👤</span>
            </div>
            <div className="absolute inset-0 rounded-full border-2 border-arena-gold/30 animate-ping" />
          </div>
          ...
          {activeDuel.stake !== '0' && (
            <div className="rounded-full bg-arena-gold/10 border border-arena-gold/30 px-3 py-1">
              <p className="text-arena-gold text-sm font-medium">₦{activeDuel.stake} stake</p>
            </div>
          )}
        </div>
      </div>
