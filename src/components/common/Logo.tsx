...
// Split the glyphs for the display "monument" lockup: the capital A (rendered
// as a carved-stone beacon-tower) vs. the rest of the word ("rena").
const A_GLYPH_CUT = ARENA_PATH.indexOf('M129.99');
const A_PATH = ARENA_PATH.slice(0, A_GLYPH_CUT);
const RENA_PATH = ARENA_PATH.slice(A_GLYPH_CUT);
// Ember flame/curl path (same as CurlMark), viewBox 56×66.
const FLAME_PATH =
  'M31 3 C 39 15, 47 25, 44 40 C 41.5 52.5, 30 60, 19.5 55.5 C 11 51.8, 8.5 41.5, 14.5 35.5 C 19.5 30.5, 27 32, 27.5 39.5 C 30.5 37, 31 30.5, 26.5 26 C 34.5 21, 33.5 12, 31 3 Z';
...
/**
 * The MONUMENT lockup (large / display fidelity) — the capital A rendered as a
 * carved-stone ancient beacon-tower: stacked ashlar masonry, softened Pharos
 * tier ledges, a lit high window (the counter) and a lit arched doorway at the
 * base, crowned by a bronze fire-dish + beacon flame; "rena" stays the glowing
 * wordmark. Use on big surfaces (auth hero, splash) — the detail smears below
 * ~40px, so the header/favicon use <TorchLockup>/<CurlMark> instead. Geometry
 * mirrors the OG render exactly (OG coordinate space; viewBox frames it tight).
 * `capHeight` is the "rena" cap height in px.
 */
export function ArchaicLockup({
  capHeight = 48,
  className,
}: {
  capHeight?: number;
  ...
  const id = useId().replace(/:/g, '');
  const T = 'translate(379,282) scale(1.1)';
  const AX = 432;
  const RIM = 267;
  const FS = 1.7;
  const FTX = (AX - 24.5 * FS).toFixed(1);
  const FTY = (RIM - 59 * FS).toFixed(1);
  const CAP_OG = 110; // wordmark cap height in OG units (100 × 1.1 scale)
  const courseYs: number[] = [];
  for (let y = 12; y < 100; y += 12.5) courseYs.push(y);
  return (
    <svg
      width={(capHeight * 470) / CAP_OG}
      height={(capHeight * 250) / CAP_OG}
      viewBox="365 150 470 250"
      fill="none"
      role="img"
      aria-label={BRAND_NAME}
      className={className}
    >
      <defs>
        ...
        <linearGradient id={`st-${id}`} x1="0" y1="0" x2="0.25" y2="1">
          <stop offset="0%" stopColor="#EBB877" />
          <stop offset="40%" stopColor="#C98A46" />
          <stop offset="100%" stopColor="#8A5A28" />
        </linearGradient>
        <linearGradient id={`dsh-${id}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#E7A863" />
          <stop offset="48%" stopColor="#A9702F" />
          <stop offset="100%" stopColor="#6E441C" />
        </linearGradient>
        <linearGradient id={`fl-${id}`} x1="0.2" y1="0.05" x2="0.75" y2="1">
          <stop offset="0%" stopColor={GOLD} />
          <stop offset="42%" stopColor={AMBER} />
          <stop offset="78%" stopColor={EMBER} />
          <stop offset="100%" stopColor={EMBER_DEEP} />
        </linearGradient>
        <radialGradient id={`lw-${id}`} cx="50%" cy="42%" r="62%">
          <stop offset="0%" stopColor="#FFD87A" stopOpacity="0.98" />
          <stop offset="52%" stopColor="#C77A28" stopOpacity="0.9" />
          <stop offset="100%" stopColor="#2A1608" stopOpacity="0.95" />
        </radialGradient>
        <clipPath id={`ac-${id}`}>
          <path d={A_PATH} transform={T} />
        </clipPath>
      </defs>

      {/* beacon flame */}
      <g transform={`translate(${FTX}, ${FTY}) scale(${FS})`}>
        <path d={FLAME_PATH} fill={`url(#fl-${id})`} />
        <path
          d="M33 9 C 39 18, 43.5 26, 41.5 36"
          stroke={WHITEHOT}
          strokeOpacity="0.55"
      ...
      {/* bronze fire-dish on the A's flat apex */}
      <g>
        <path
          d={`M${AX - 26} ${RIM + 1} L${AX + 26} ${RIM + 1} Q${AX + 22} ${RIM + 18} ${AX} ${RIM + 18} Q${AX - 22} ${RIM + 18} ${AX - 26} ${RIM + 1} Z`}
          fill={`url(#dsh-${id})`}
        />
        <ellipse cx={AX} cy={RIM + 1} rx="26" ry="5.4" fill="#7A4E20" />
        <ellipse cx={AX} cy={RIM - 0.4} rx="26" ry="5.4" fill="#E7B575" />
        <ellipse cx={AX} cy={RIM} rx="18" ry="3.4" fill="#FFC24E" />
      </g>

      {/* stone tower A */}
      <path d={A_PATH} transform={T} fill={`url(#st-${id})`} />

      {/* lit arched doorway (in the leg-gap) */}
      <g transform={T}>
        <path d="M40 100 L40 85 Q40 76 48 76 Q56 76 56 85 L56 100 Z" fill={`url(#lw-${id})`} />
        <path
          d="M40 85 Q40 76 48 76 Q56 76 56 85"
          fill="none"
          stroke="#3E2711"
          strokeOpacity="0.6"
          strokeWidth="1.4"
      ...
      {/* ashlar masonry + Pharos tiers + lintel (clipped to the A) */}
      <g clipPath={`url(#ac-${id})`}>
        <g transform={T}>
          {courseYs.map((y, i) => (
            <g key={`c${i}`}>
              <line x1="-6" y1={y} x2="102" y2={y} stroke="#3E2711" strokeOpacity="0.5" strokeWidth="1.9" />
              <line
                x1="-6"
                y1={y + 1.5}
                x2="102"
          ...
          {courseYs.flatMap((y, i) => {
            const off = (i % 2) * 8;
            const out = [];
            for (let x = -6 + off; x < 102; x += 16)
              out.push(
                <line
                  key={`j${i}-${x}`}
                  x1={x}
          ...
          <rect x="-6" y="35" width="108" height="1.6" fill="#F6D49A" fillOpacity="0.3" />
          <rect x="-6" y="36.6" width="108" height="3" fill="#3E2711" fillOpacity="0.38" />
          <rect x="-6" y="70" width="108" height="1.6" fill="#F6D49A" fillOpacity="0.3" />
          <rect x="-6" y="71.6" width="108" height="3" fill="#3E2711" fillOpacity="0.38" />
          <rect x="-6" y="55.5" width="108" height="9.5" fill="#7A4E20" fillOpacity="0.38" />
          <path d="M0 100 L60 100 L96.84 0 L70 0 Z" fill="#3E2711" fillOpacity="0.16" />
        </g>
      </g>

      {/* lit high window (the A's counter kept glowing) */}
      <g transform={T}>
        <path d="M42.84 16 L55 55.5 L30.7 55.5 Z" fill={`url(#lw-${id})`} />
        <circle cx="42.84" cy="42" r="4.5" fill="#FFE9A8" fillOpacity="0.85" />
      </g>

      {/* "rena" wordmark */}
      <path d={RENA_PATH} transform={T} fill={`url(#w-${id})`} />
    </svg>
  );
}
